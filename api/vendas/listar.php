...
$query = $pdo->query("SELECT * FROM $tabela where data_venc >= '$dataInicial' and data_venc <= '$dataFinal' and pago LIKE '$status' and produto != 0 and usuario_lanc = '$id_usuario' and id_conta = '$id_conta' ORDER BY pago asc, data_venc asc");
...
if($data_venc < $data_hoje and $pago != 'Sim'){
	...
	$classe_debito = '';
		 ...
		 echo '<li>';
             echo '<a href="#" class="item-link item-content" onclick="editarVendas('.$id.', \''.$descricao.'\', \''.$valorF.'\', \''.$nome_pessoa.'\', \''.$data_vencF.'\', \''.$data_pgtoF.'\', \''.$tumb_arquivo.'\', \''.$nome_usuario_lanc.'\', \''.$nome_usuario_pgto.'\', \''.$foto.'\', \''.$ext.'\', \''.$oc.'\')">'; 
