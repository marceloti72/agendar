...
if($pgto_api != 'Sim'){
    ...
    //echo "<script>window.location='{$url_sistema}barbearia/pagamentos/pagamento_aprovado.php?id_agd=$id_conta'</script>";
...
<div id="info_pagamento" style="text-align: center;"> 
        ...
        <?php if($porc_servico > 0){ ?>
         ...
         <div style="margin-top: -8px; margin-bottom: 8px; font-size: 14px"><a href="<?php echo $id_conta ?>/<?php echo $porc_servico ?>">Pagar <?=$porc_servico;?>% </a> / <a href="/<?php echo $id_conta ?>/100"> Pagar 100% </a> </div>
        <?php } } ?>
        
        <div style=" margin-bottom: 8px; font-size: 14px"><b>Agendado</b> Data:<?=$dataF;?> / Hora <?=$horaF;?><br></div>

        <?php if($porc_servico <= 0){ ?>
        <div style="margin-bottom: 8px; font-size: 16px"><a href="<?php echo $url_sistema ?>pagamentos/pagamento_aprovado.php?id_agd=<?php echo $id_conta ?>"><b>>>CLIQUE AQUI<<</b></a> <span style="font-size: 14px"> para confirmar e efetuar o pagamento no local!</span></div>
        <?php } ?>
