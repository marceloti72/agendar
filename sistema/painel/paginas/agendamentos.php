    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content bg-white dark:bg-slate-800 rounded-xl shadow-xl">
            <div class="modal-header bg-blue-600 text-white flex justify-between items-center px-6 py-4">
                ...
                <button type="button" data-dismiss="modal" class="text-white hover:text-gray-200 text-2xl" aria-label="Close">
                ...
                <div class="modal-body p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label for="servico2" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Serviço</label> 
                                <select class="form-control sel3" id="servico2" name="servico" style="width:100%;" required></select>    
                            </div>
                            <div>
                                <label for="cupom" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Cupom de Desconto</label>
                                <select class="form-control" name="cupom" id="cupom">
                                    <option value="">Nenhum</option>
                                    <?php
                                    $query_cupons = $pdo->prepare("SELECT * FROM cupons WHERE id_conta = :id_conta AND data_validade >= CURDATE() AND (usos_atuais < max_usos OR max_usos = 0) ORDER BY codigo ASC");
                                    $query_cupons->execute([':id_conta' => $id_conta]);
                                    foreach ($query_cupons->fetchAll(PDO::FETCH_ASSOC) as $cupom) {
                                        ...
                                        echo "<option value=\"".htmlspecialchars($cupom['codigo'])."\">".htmlspecialchars($cupom['codigo'])." ({$desconto})</option>";
                                    }
                                    ?>
                                </select>
                            </div>
                             <div>
                                <label for="obs" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Observações <small>(Opcional)</small></label> 
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="funcionario_modal" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Profissional</label>     
                            ...
                            <div>
                                <label for="data-modal" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Data do Agendamento</label> 
                                <input type="date" class="form-control" name="data" id="data-modal" onchange="mudarData()"> 
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Horários Disponíveis</label> 
                                <div id="listar-horarios" class="p-3 border rounded-md min-h-[100px] bg-gray-50 dark:bg-slate-700 dark:border-slate-600">
                                    <small class="text-gray-500 dark:text-gray-400">Selecione um profissional e uma data.</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <small><div id="mensagem" class="text-center mt-3"></div></small>                     
                </div>
                <div class="modal-footer bg-gray-50 dark:bg-slate-900 px-6 py-3">
