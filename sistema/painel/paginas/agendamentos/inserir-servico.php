...
$cliente_id = isset($_POST['cliente_agd']) ? filter_var($_POST['cliente_agd'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]) : 0;
$data_pgto_prevista = isset($_POST['data_pgto']) ? $_POST['data_pgto'] : '';
$id_agd = isset($_POST['id_agd']) ? filter_var($_POST['id_agd'], FILTER_VALIDATE_INT) : 0;
$valor_serv_postado = isset($_POST['valor_serv_agd']) ? filter_var($_POST['valor_serv_agd'], FILTER_VALIDATE_FLOAT, FILTER_FLAG_ALLOW_FRACTION) : 0.0;
$funcionario_id = isset($_POST['funcionario_agd']) ? filter_var($_POST['funcionario_agd'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]) : 0;
$servico_id = isset($_POST['servico_agd']) ? filter_var($_POST['servico_agd'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]) : 0;
...
try {
    ...
    if ($valor_final_restante > 0) {
        ...
        $query_rec_res = $pdo->prepare("INSERT INTO receber
            (descricao, tipo, valor, data_lanc, data_venc, data_pgto, usuario_lanc, usuario_baixa, foto, pessoa, cliente, pago, servico, funcionario, obs, pgto, id_agenda, id_conta, referencia)
            VALUES
            (:desc, 'Serviço', :val, CURDATE(), :dvenc, :dpgto, :user_lanc, :user_baixa, 'sem-foto.jpg', :pessoa, :cliente, :pago, :serv, :func, :obs, :pgto, :id_agd, :idc, :ref)");
    ...
    $query_check_exist = $pdo->prepare("SELECT id FROM receber WHERE referencia = :ref AND id_conta = :idc AND tipo = 'Serviço' ORDER BY id DESC LIMIT 1"); // Procura por serviço com essa ref
    $query_check_exist->execute([':ref' => $id_agd, ':idc' => $id_conta_corrente]);
    $existing_receber = $query_check_exist->fetch(PDO::FETCH_ASSOC);

    if ($existing_receber) {
        $id_receber_principal = $existing_receber['id'];
        // ATUALIZA registro existente
         $query_upd_rec = $pdo->prepare("UPDATE receber SET
                descricao = :desc,
                valor = :val,
                data_venc = :dvenc, -- Atualiza vencimento caso mudado no POST
                data_pgto = :dpgto,
                usuario_baixa = :user_baixa,
                pago = :pago,
                pgto = :pgto,
                pessoa = :pessoa, -- Garante que está correto
                servico = :serv,
                funcionario = :func,
                obs = :obs
            WHERE id = :id_rec AND id_conta = :idc");
        $query_upd_rec->execute([
            ':desc' => $descricao_final_receber,
            ':val' => $valor_final_receber,
            ...
            ':func' => $funcionario_id,
            ':obs' => $obs,
            ':id_rec' => $id_receber_principal,
            ':idc' => $id_conta_corrente
        ]);
         error_log("[CONCLUIR AGD {$id_agd}] Lançamento principal 'receber' (ID: {$id_receber_principal}) ATUALIZADO. Valor: {$valor_final_receber}, Pago: {$pago_receber}.");

    } else {
        // INSERE novo registro principal
        // ** CONFIRME O NOME DA COLUNA: 'pessoa' ou 'cliente' **
        $query_rec_princ = $pdo->prepare("INSERT INTO receber
            (descricao, tipo, valor, data_lanc, data_venc, data_pgto, usuario_lanc, usuario_baixa, foto, pessoa, pago, servico, funcionario, obs, pgto, id_agenda, id_conta, referencia)
            VALUES
            (:desc, 'Serviço', :val, CURDATE(), :dvenc, :dpgto, :user_lanc, :user_baixa, 'sem-foto.jpg', :pessoa, :pago, :serv, :func, :obs, :pgto, :id_agd, :idc, :ref)");
