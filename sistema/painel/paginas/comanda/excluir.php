...
$response = ['success' => false, 'message' => 'Erro desconhecido ao excluir comanda.'];
...
try {
    ...
    $query = $pdo->prepare("SELECT cliente, funcionario, data, hora, servico, hash, id FROM agendamentos WHERE comanda_id = :id AND id_conta = :id_conta");
    $query->execute([':id' => $id, ':id_conta' => $id_conta]);
    $res = $query->fetchAll(PDO::FETCH_ASSOC);

    if (count($res) == 0) {
        throw new Exception("Agendamento não encontrado para a comanda ID {$id}.");
    }
    ...
    // 3. Verifica e deleta registros de uso de serviços para assinantes
    $query_receber = $pdo->prepare("SELECT cliente, valor2, tipo, servico FROM receber WHERE comanda = :id_comanda AND id_conta = :id_conta");
    $query_receber->execute([':id_comanda' => $id, ':id_conta' => $id_conta]);
    $receber_itens = $query_receber->fetchAll(PDO::FETCH_ASSOC);

    foreach ($receber_itens as $item) {
        ...
        if ($id_assinante !== null && $id_assinante > 0 && $valor2 == 0 && $tipo === 'Serviço') {
            ...
            if ($query_del_uso->rowCount() <= 0) {
                error_log("Aviso: Nenhum registro encontrado em 'assinantes_servicos_usados' para id_assinante {$id_assinante}, id_servico {$id_servico} ao excluir comanda ID {$id}.");
            }
        }
    }

    // 4. Exclui registros relacionados
    $query = $pdo->prepare("DELETE FROM agendamentos WHERE comanda_id = :id AND id_conta = :id_conta");
    $query->execute([':id' => $id, ':id_conta' => $id_conta]);
    if ($query->rowCount() <= 0) {
        error_log("Aviso: Nenhum agendamento excluído para comanda ID {$id}.");
    }

    $query = $pdo->prepare("DELETE FROM horarios_agd WHERE agendamento = :id_agenda AND id_conta = :id_conta");
    $query->execute([':id_agenda' => $id_agenda, ':id_conta' => $id_conta]);
    if ($query->rowCount() <= 0) {
        error_log("Aviso: Nenhum horário adicional excluído para agendamento ID {$id_agenda}.");
    }

    $query = $pdo->prepare("DELETE FROM comandas WHERE id = :id AND id_conta = :id_conta");
    $query->execute([':id' => $id, ':id_conta' => $id_conta]);
    if ($query->rowCount() <= 0) {
        throw new Exception("Falha ao excluir comanda ID {$id}.");
    }

    $query = $pdo->prepare("DELETE FROM pagar WHERE comanda = :id AND id_conta = :id_conta");
    $query->execute([':id' => $id, ':id_conta' => $id_conta]);
    if ($query->rowCount() <= 0) {
        error_log("Aviso: Nenhuma comissão excluída para comanda ID {$id}.");
    }

    $query = $pdo->prepare("DELETE FROM receber WHERE comanda = :id AND id_conta = :id_conta");
    $query->execute([':id' => $id, ':id_conta' => $id_conta]);
    if ($query->rowCount() <= 0) {
        error_log("Aviso: Nenhum registro excluído da tabela 'receber' para comanda ID {$id}.");
    }
    ...
    error_log("Erro PDO em excluir_comanda.php: " . $e->getMessage() . " | ID Comanda: " . $id);
    echo json_encode($response); exit;
} catch (Exception $e) {
    $pdo->rollBack();
    $response['message'] = 'Erro: ' . $e->getMessage();
    error_log("Erro Geral em excluir_comanda.php: " . $e->getMessage() . " | ID Comanda: " . $id);
    echo json_encode($response); exit;
}
