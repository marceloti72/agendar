...
$query = $pdo->query("SELECT * FROM $tabela where data_venc >= '$dataInicial' and data_venc <= '$dataFinal' and pago LIKE '$status' and id_conta = '$id_conta' ORDER BY pago asc, data_venc asc");
...
<td>
		<a href="#" class="btn btn-primary btn-xs" onclick="editar('{$id}','{$descricao}', '{$pessoa}', '{$valor}', '{$data_venc}', '{$data_pgto}', '{$tumb_arquivo}', '{$funcionario}')" title="Editar Dados"><i class="fe fe-edit"></i></a>

		<a href="#" class="btn btn-info btn-xs" onclick="mostrar('{$descricao}', '{$valorF}', '{$data_lancF}', '{$data_vencF}',  '{$data_pgtoF}', '{$nome_usuario_lanc}', '{$nome_usuario_pgto}', '{$tumb_arquivo}', '{$nome_pessoa}', '{$foto}', '{$nome_func}', '{$telefone_func}', '{$chave}')" title="Ver Dados"><i class="fe fe-search"></i></a>
		...
		<li class="dropdown head-dpdn2" style="display: inline-block;">
		<a title="Baixar Conta" href="#" class="btn btn-primary btn-xs dropdown-toggle {$visivel}" data-toggle="dropdown" aria-expanded="false"><i class="fe fe-dollar-sign"></i></a>
		...
		<a href="http://api.whatsapp.com/send?1=pt_BR&phone=$whats&text=" target="_blank" title="Abrir Whatsapp" class="btn btn-success btn-xs"><i class="fa fa-whatsapp fa-2x" style = 'font-size: 16px;'></i></a>
