...
<body>	
	...
	<div class="mx-2" >
		...
		<?php 
		$total_entradas = 0;
		$query = $pdo->query("SELECT * FROM receber where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and tipo = 'Serviço' and pago = 'Sim' and servico LIKE '$servico' and pgto LIKE '$pgto'  ORDER BY data_pgto asc");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){
		 ?>
